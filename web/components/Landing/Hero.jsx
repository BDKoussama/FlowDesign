    ...
    return(
        <Container> 
            <div className='hero w-full text-center z-2 ' ref = {hero}>
                <h1 className='text-[8vw] md:text-[6vw] font-bold mt-0 leading-snug w-10/12 mx-auto text-slate-200 capitalize'> 
                    {headingSpans.current.length !== 0 && headingSpans.current.map((item , index)  => (
                        <span className="mx-2 overflow-hidden block" key={`${item}-${index}`}>
                            <span className={`translate-y-[110%] block translate-z-px will-change-transform ${index === 0 ? 'highlight-text' : ''}`}>{item}</span>
                        </span>
                    ))} 
                </h1>
